...
export default function ResourceCard({title, description, image, price, className}: ResourceCardProps): JSX.Element {
    ...
    return (
        <div className={`flex flex-row justify-start items-stretch gap-x-4 rounded-xl shadow-md
         border-2 border-neutral-100 p-1 cursor-pointer ${className}`}>

            <Image src={image} alt={''}
                   className={'rounded-l-xl max-w-[35%] max-h-[25rem] min-h-[20rem] object-cover'}/>

            <div className={'flex-1 flex flex-col justify-start items-start gap-y-1 p-4'}>
                <h3 className={'text-2xl font-bold text-neutral-600'}>{title}</h3>
                <p className={'flex-1 text-sm font-medium text-neutral-500'}>{description}</p>
                ...
                <div className={'self-stretch flex flex-row justify-between items-center'}>
                    ...
                    <Link href={'/resources'} passHref className={'flex flex-row justify-start items-center gap-2' +
                        ' relative pb-1'}>
